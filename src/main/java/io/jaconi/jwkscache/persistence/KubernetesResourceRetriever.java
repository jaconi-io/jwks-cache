...
public class KubernetesResourceRetriever extends PersistingResourceRetriever {
	...
	protected void store(URL url, Resource resource) throws IOException {
		...
		if (existing == null) {
			...
			existing.putDataItem(key, data);

			try {
				core.replaceNamespacedConfigMap(name, namespace, existing).execute();
			} catch (ApiException e) {
				throw new IOException("failed to update Kubernetes ConfigMap %s/%s".formatted(namespace, name), e);
			}
		}
	}
