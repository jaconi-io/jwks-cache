...
public class KubernetesResourceRetriever extends PersistingResourceRetriever {
	...
	protected void store(URL url, Resource resource) throws IOException {
		...
		var key = getKey(url);
		var data = objectMapper.writeValueAsString(resource);
		...
		if (existing == null) {
			...
			var escapedData = JsonStringEncoder.getInstance().quoteAsString(data);
	...
	protected Resource load(URL url) throws IOException {
		...
		var data = cm.getData().get(getKey(url));
		if (data == null) {
			throw new IOException("existing ConfigMap %s/%s does not contain %s".formatted(namespace, name, url));
		}
