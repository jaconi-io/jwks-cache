...
@SpringBootTest(properties = {
		...
		"jaconi.jwks.outage-tolerance.enabled = true",
		"jaconi.jwks.urls = http://localhost:8082"
})
class OutageToleranceEnabledTest {
	private final ClientAndServer client;

	@Autowired
	private WellKnownController wellKnownController;

	public OutageToleranceEnabledTest(ClientAndServer client) {
		this.client = client;
	}
	...
	void testOutageTolerance() {
		...
		// Expect a non-empty response despite the cache being disabled.
