...
class KubernetesResourceRetrieverTest {
	...
	@Test
	void storeToExistingEmptyCM() throws ApiException, IOException {
		var getReq = mock(CoreV1Api.APIreadNamespacedConfigMapRequest.class);
		when(getReq.execute()).thenReturn(new V1ConfigMap());
		when(core.readNamespacedConfigMap("my-cm", "my-ns")).thenReturn(getReq);

		var cmCaptor = ArgumentCaptor.forClass(V1Patch.class);
		when(core.patchNamespacedConfigMap(eq("my-cm"), eq("my-ns"), cmCaptor.capture()))
				.thenAnswer(_ -> mock(CoreV1Api.APIpatchNamespacedConfigMapRequest.class));

		var retriever = new KubernetesResourceRetriever(core, JSON_MAPPER, "my-ns", "my-cm");
		retriever.store(URI.create("https://example.com").toURL(), new Resource("\"foo\"", null));

		var cm = cmCaptor.getValue();
		assertThat(cm.getValue()).contains("foo");
	}
